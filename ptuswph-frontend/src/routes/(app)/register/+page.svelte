<script lang="ts">
    ...
    const registerUser = async (e: any) => {
        ...
        if (!res.ok) {
            alert(
                "Nieprawidłowy login lub hasło. (min 2 znaki, login musi być unikalny)"
            );
            return;
        }
